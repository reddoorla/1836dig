...
The one-page site for [1836dig.com](https://www.1836dig.com/) — 1836 Digital
Investment Group — converted off Webflow onto the Reddoor stack: SvelteKit 2 /
Svelte 5 / Tailwind 4 on adapter-netlify, with **no CMS**. There is exactly one
page and its content is hardcoded in `src/routes/+page.svelte`; the contact form
posts to that route's action, which wraps the central dashboard ingest
(`@reddoorla/maintenance/forms`) with the standard honeypot, fill-timing screen
and optional Turnstile.

Commands: `pnpm dev`, `pnpm build`, `pnpm check` (svelte-check), `pnpm lint`
(prettier then eslint), `pnpm test:smoke` (Playwright). There is no `pnpm verify`
aggregate here — CI is the shared fleet caller in `.github/workflows/ci.yml`, so
run the four yourself before pushing. `src/routes/dev/` holds the a11y fixture
routes the fleet audit visits; they are not part of the site.

**Every asset is self-hosted under `static/` on purpose** — the background
photo, the star logo, the favicon and the Proxima Nova webfont, with
Baskervville from `@fontsource`. Nothing references the Webflow CDN, which is
the whole reason the conversion was worth doing. Do not reintroduce a remote
asset URL.
